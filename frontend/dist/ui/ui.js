...
export const UI = {
    ...
    atualizarDashboard() {
        ...
        if (txtReceitas)
            txtReceitas.textContent = formatarMoeda(receitas);
        if (txtDespesas)
            txtDespesas.textContent = formatarMoeda(despesas);
        if (txtSaldo) {
    ...
    configurarBotoesAcao() {
        ...
        document.querySelectorAll('.btn-editar').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (transacao)
                    this.ativarModoEdicao(transacao);
            });
        });
        // Cliques em Excluir
    ...
    ativarModoEdicao(transacao) {
        ...
        document.getElementById('transacao-id').value = String(transacao.id);
        document.getElementById('transacao-descricao').value = transacao.descricao;
        document.getElementById('transacao-valor').value = String(transacao.valor);
        document.getElementById('transacao-tipo').value = transacao.tipo;
        document.getElementById('transacao-categoria').value = transacao.categoria_id ? String(transacao.categoria_id) : '';
    },
    desativarModoEdicao() {
